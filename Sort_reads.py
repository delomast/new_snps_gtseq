...
def Sort(queue, samples, name_fwd, het_ratio):
	...
	for indiv in samples:
		haplo_indiv = []
		for marker in name_fwd:		#iterate through markers - could assess all markers at the same time, but memory demand would be high
			length = len(name_fwd[marker])	#get length of fwd primer
			unique = {}			#dictionary to count depth of each sequence
			for read in SeqIO.parse(indiv + '.fastq', 'fastq'):
				string_read = str(read.seq)
